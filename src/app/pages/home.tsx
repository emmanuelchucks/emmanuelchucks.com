...
export function Home(): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`grid grid-flow-col grid-cols-[auto_max-content] grid-rows-[repeat(2,_max-content)] items-center gap-x-10 gap-y-1`}
        >
        ...
        <aside
          className={`mt-6 flex flex-row flex-wrap gap-x-6 gap-y-2 font-semibold text-neutral-800 dark:text-neutral-200`}
        >
        ...
        {posts.length > 0 && (
          <section className="mt-24">
            <h2
              className={`text-xl font-semibold text-neutral-600 dark:text-neutral-400`}
            >
              Posts
            </h2>
            <ul className="mt-6 grid gap-y-14">
              {posts.map((post) => (
                <li key={post.id} className="grid gap-y-3">
                  ...
                  <PostMeta post={post} />
                  <p
                    className={`line-clamp-3 text-neutral-700 dark:text-neutral-300`}
                  >
                    {post.description}
                  </p>
                </li>
