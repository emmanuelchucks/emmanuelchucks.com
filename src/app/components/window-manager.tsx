...
export const activeWindowAtom = createAtom<WindowStore | undefined>(
...
export function addFloatingWindow(windowStore: WindowStore): void {
  ...
  const updatedFloatingWindows = new Set([...currentFloatingWindows, windowStore]);
...
export function addDockedWindow(windowStore: WindowStore): void {
  ...
  const remainingWindows = floatingWindowsArray.filter((_, index) => index !== currentWindowIndex);

  const nextFloatingWindow = remainingWindows.at(-1);
  const updatedDockedWindows = new Set([...currentDockedWindows, windowStore]);
